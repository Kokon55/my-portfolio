...
export default function CaseRunner({ caseDef }: { caseDef: Case }) {
  ...
  if (isLockedAct) {
    return <PaywallModal caseTitle={caseDef.title} />;
  }
      ...
      <header className="sticky top-0 z-30 backdrop-blur-md bg-slate-950/80 border-b border-slate-800">
        <div className="max-w-2xl mx-auto px-4 py-3 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[10px] uppercase tracking-widest px-2 py-1 rounded ${actBadge[act.type]}`}>
              {actLabel[act.type]}
            </span>
          </div>
          <ProgressBar
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4 pb-32">
        ...
        {isPaywalled && (
          <motion.div
            ...
            className="rounded-2xl border border-amber-accent/40 bg-gradient-to-br from-amber-950/30 to-slate-950/60 p-5 text-center"
          >
            <div className="text-amber-accent text-2xl mb-1">🔒</div>
            <p className="text-sm text-slate-200 mb-3">
              ここから先は——捜査の核心、推理パート、真相と教訓が待っています。
            </p>
            <Link
              href="/purchase"
              className="inline-block px-5 py-2.5 rounded-xl bg-amber-accent text-slate-900 font-bold active:scale-95"
            >
              全6ケースを解放する
            </Link>
          </motion.div>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-30 backdrop-blur-md bg-slate-950/90 border-t border-slate-800">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-2">
          ...
          {!isPaywalled ? (
            ...
            <Link
              href="/purchase"
              className="flex-1 px-4 py-3 rounded-xl bg-gradient-to-r from-amber-accent to-yellow-600 text-slate-900 font-bold text-center active:scale-95"
            >
              続きを購入する
            </Link>
          )}
